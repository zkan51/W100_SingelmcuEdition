...
// 经销商信息
void WriteFalsh_AgencyInfo(void)
{
	u8 i;
	FLASHStatus = FLASH_COMPLETE;
	MemoryProgramStatus = PASSED; 
	...
	NbrOfPage = (EndAddr[12] - StartAddr[12]) / FLASH_PAGE_SIZE; //页面擦除子程序
	FLASHStatus = FLASH_ErasePage(StartAddr[12] + (FLASH_PAGE_SIZE * NbrOfPage));

	FlashAddress = StartAddr[12];

	FLASHStatus = FLASH_ProgramWord(FlashAddress, Time);	
	FlashAddress = FlashAddress + 4;
	for(i=0;i<4;i++)
	{
...
void ReadFalsh_AgencyInfo(void)
{
	u8 i;
	FlashAddress = StartAddr[12]; 
	Time = *(unsigned long*)FlashAddress; 
	FlashAddress += 4;
	
	for(i=0; i<16; i++)
	{
		AgencyName[i] = *(unsigned char*)(FlashAddress+i);
	}
}
